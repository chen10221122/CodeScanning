...
export interface IDicatorConfig {
  ...
  /**指标筛选的新配置icon */
  hasNewIcon?: boolean;
  /**区域数据浏览器新配置icon--因为有已写好的逻辑还是分开配置，省的再写渲染 */
  iconList?: string[];
...
export interface ItemConfig extends selectItem {
  ...
  isCustom?: boolean;
  /**自定义支持编辑 */
  onEditCustomNode?: Function;
  /**自定义删除 */
  onDeleteCustomNode?: Function;
  /**模板区分 defaultTemplate:样例指标 */
  templateType?: string;
  canAdd?: boolean;
  onAddCustom?: Function;
  /**区域数据浏览器需要这个字段 */
    ...
    children: [
      {
        ...
        active: true,
        describe:
          '2019年及以后人口采用常住人口口径,在数据采集过程中存在部分地区未披露常住人口,则采集户籍人口,可能会造成部分年份数据间差距较大。',
        key: '人口1',
