...
/** isLastMonth: 只显示近一月,isLastWeek: 近一周  */
export default (isLastMonth: boolean, missVCA = false, isLastWeek = false) => {
  const filterRef = useRef<HTMLDivElement | null>(null);
  const [openUpdate, setOpenUpdate] = useImmer<TipInfo>({
    isUpdate: true,
    days: isLastWeek ? 7 : 30,
  });
  ...
  /** 处理表格中的色块显示 */
  const handleTblCell = useMemoizedFn(({ isUpdateItem, onClick, defaultClassName, isMissVCA = false }: CellProps) => {
    ...
    if (isMissVCA) result.className = `${result.className || ''} missVCA`;
    return result;
  });
  ...
  const UpdateTipCref = useMemo(() => {
    ...
    return (
      <UpdateTipBottom className="update-bottom-tip">
        ...
        {missVCA ? <div className="text missVCA-img">计算过程中涉及未披露数据的指标</div> : null}
        <Popover
          placement="top"
          content={
            <HoverText>
              <div className="note-title">提示数据存在的特殊口径，包括如下3类：</div>
              <div>1、是否包含功能区等特殊区域；</div>
              <div>2、数据是否存在特殊的统计口径，如：市本级、市级等；</div>
              <div>3、2019年及以后人口采用常住人口口径，若未披露则以户籍口径替代；</div>
            </HoverText>
          }
          limitContent={false}
          // getPopupContainer={() => document.querySelector('.area-economy-table-wrap') as HTMLElement}
        >
