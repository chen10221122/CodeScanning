...
import { sourceValueObj, unitObj } from '@/pages/area/areaDebt/components/filter/indicator';
...
import { formateData } from '@/pages/area/areaDebt/config';
...
export default function CalculateIndicModal({
  visible,
  info,
  data: initData,
  onClose,
  ...
  const { loading, data, run } = useData({ isCalculateIndic: true });
  useEffect(() => {
    /** 传页面上显示的指标名称 */
    const { indicName, year, regionCode, pageCode, unit } = info;
    if (regionCode && indicName && pageCode) {
      run({
        ...fixedParams,
        indicName: `${indicName}${unit}`,
        endDate: year,
        regionCode,
        pageCode,
      });
  ...
  const res = useMemo(() => formateData(data, info.indicName, initData), [data, initData, info.indicName]);
      ...
      columns: [
        ...
        },
        {
          ...
          onCell: (record: any) => {
            ...
            const isNewAdd = [1, 3].includes(updateType);
            if (updateType && record?.mValue) {
              return {
                onClick: () => {
                  const removeUnitIndic = record?.indicName?.split('(')?.[0] || record?.indicName;
                  updateType
                    ? openModal(
                        {
                          title: `${info?.regionName}_${record?.indicName}`,
                          regionCode: info?.regionCode || '',
                          indicName: removeUnitIndic,
                          indicName2: record?.addName || sourceValueObj[record?.indicName] || record?.indicName,
                          unit: `${unitObj[removeUnitIndic] || ''}`,
                          year: info.year,
                          pageCode: info.pageCode,
                          regionName: info.regionName,
                        },
                        specialIndicList.includes(removeUnitIndic),
                      )
                    : record?.posId && openDataSource
                    ? openDataSource({ posIDs: record.posId, jumpToPdf: true })
                    : goToTrancePage(record?.guid);
                },
                className: isNewAdd ? 'first-update' : 'cell-class',
              };
            }
  ...
  }, [res?.data, expandable, getFoldIcon, goToTrancePage, openModal, info, openDataSource]);
  ...
  return (
    <ModalStyle visible={visible} {...setting}>
      <div className="modal-content app-scrollbar-small trance-modal" ref={domRef}>
        <Loading show={loading || !hasData} style={{ height: '226px' }}>
          {hasData ? (
            <>
              {/* @ts-ignore */}
              <TableStyle {...TblSetting} alignSync />
              {!fixbottom ? (
                <div className="tips">
                  <div>说明：{res?.tips}</div>
                  <div>{res?.detail}</div>
...
const TableStyle = styled(Table)`
  ...
  .ant-table-container .ant-table-tbody > tr.dzh-table-clicked-row {
    .cell-class {
      background-color: #ffd6d6 !important;
    }
    .first-update {
