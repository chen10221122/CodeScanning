...
import { useCtx } from '@pages/area/landTopic/provider';
...
export default function useIndicator() {
  ...
  const [defaultIndicator, setDefaultIndicator] = useState<any[]>([]);
  ...
  const { run } = useRequest(getIndexTree, {
    ...
    onSuccess: ({ data }: any, [type]) => {
      ...
      setDefaultIndicator(detailDefault);
      update((draft) => {
