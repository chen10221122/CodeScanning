...
import { getNodeByKey } from '@pages/area/landTopic/components/transferSelect/utils';
...
export default function useTree({ checkedNodes, setCheckedNodes, setExpandedKeys }: Props) {
  const {
    state: { formatTreeData, checkAllNodes, checkAllKeys, maxSelect, curEditPlan, checkMaxLimit },
    update,
  } = useCtx();
  ...
  /* 当切换编辑方案时，把节点禁用计算锁打开，重新走一遍 节点的禁用和解除禁用逻辑 */
  useEffect(() => {
    canDisableRef.current = canAbleRef.current = true;
  }, [curEditPlan]);
    ...
    if (checked) {
      setCheckedNodes((preState) =>
        preState.filter((preItem) =>
          changeNodes.every(({ associatedKey, key }) => {
      ...
      */
      ...
      changeNodes.forEach((changeItem) => {
        const { ignoreIndicator, key } = changeItem;
        if (!ignoreIndicator && checkedNodes.every((preItem) => preItem.key !== key)) newSelecting.push(changeItem);
      });
      /* 更新选中项前要做数量限制检查 */
      if (checkMaxLimit && newSelecting.length > maxSelect) {
        update((draft) => {
          draft.tipInfo = {
            visible: true,
            text: `勾选内容超限，最多不超过 ${maxSelect} 条！`,
            type: TipType.error,
          };
        });
      } else setCheckedNodes(newSelecting);
    }
  });

  /** 点击复选框触发 */
  const onCheck = useMemoizedFn((checkedKeys, { node }) => handleCheck(node));
