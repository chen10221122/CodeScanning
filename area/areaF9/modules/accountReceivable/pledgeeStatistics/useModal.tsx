...
import { postPledgorList, postPledgeeDetails } from '../api';
...
function removeTrailingGreaterSigns(str: string) {
  // 检查字符串是否以'>'结尾
  while (str.endsWith('>')) {
    // 如果是，去除最后一个字符
...
export default function useModal({ condition }: any) {
      ...
      case modalType.EVENT:
        arr = [
          ...
          },
          {
            ...
            align: 'left',
            render: (text: string, row: any) => {
              return <CompanyEllipsis data={row.pledgeeList} rowLimit={2} textLimitwidth={242} showTag={false} />;
            },
          },
          {
            title: '融资类型',
      ...
      case modalType.ENTERPRISE:
        arr = [
          ...
          },
          {
            ...
            align: 'left',
            render: (text: string, row: any) => {
              const { fourthGBIndustry, thirdGBIndustry, secondGBIndustry, primaryGBIndustry } = row;
              const contentStr = `${primaryGBIndustry}>${secondGBIndustry}>${thirdGBIndustry}>${fourthGBIndustry}`;
              const content = removeTrailingGreaterSigns(contentStr);
              return (
                <>
                  {fourthGBIndustry || thirdGBIndustry || secondGBIndustry || primaryGBIndustry || '-'}
                  <Popover
  ...
  const openModal = useMemoizedFn((title: string, type: modalType, module_type, name, itCode, coType) => {
    setTitle(title);
  ...
  const { loading } = useRequest(
    type === modalType.EVENT ? () => postPledgeeDetails(modalCondition) : () => postPledgorList(modalCondition),
    {
      ready: modalVisible,
