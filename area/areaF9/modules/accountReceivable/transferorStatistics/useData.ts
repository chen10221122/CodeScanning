...
import { getScreenOptions, postPledgorList } from '../api';
...
const sortKeyMap = new Map<string, string>([
  ...
  ['name', ''],
  ['fourthGBIndustry', 'GBIndustry'],
]);

export default function useData() {
  const screenRef = useRef();
  const { StaticOptions } = useStaticOptions(PageType.TRANSFEROR);
  ...
  const { run, loading: tableLoading } = useRequest(postPledgorList, {
    ready:
  ...
  const handleScreenChange = useMemoizedFn((key, selected, index) => {
    // 行业筛选项特殊处理
    if (key === 'industry') {
      setCondition((draft) => {
  ...
  const handleTableSortChange = useMemoizedFn(
    (pagination, filters, sorter, extra: { currentDataSource: []; action: any }) => {
      setCondition((draft) => {
        draft.sortKey = !sorter.order ? '' : `${sortKeyMap.get(sorter.field)}`;
