...
import {
  ...
  RankDetailApiData,
  RankDetailItem,
} from '@/pages/area/areaF9/modules/regionalOverview/areaRank/api';
...
import { transformUrl, urlJoin } from '@/utils/url';
...
export default function RankDetail() {
  const { detailModalVisible, activeDetailInfo } = useSelector((s) => ({
    detailModalVisible: s.rankModalVisible,
    activeDetailInfo: s.rankModalCurData,
  }));
  const history = useHistory();

  const { handleCloseModal } = useRankModal();
        ...
        render: (v: string | number) => {
          return (
            <div className={S.rankWrap}>
              ...
              {Number(v) > 3 ? <span className={S.rankText}>{v}</span> : null}
            </div>
          );
        },
      },
            ...
            <div
              onClick={() => {
                if (isCorrectArea(item.areaCode)) {
                  handleLimit(item.areaCode, () => {
                    history.push(
                      urlJoin(dynamicLink(LINK_AREA_ECONOMY, { key: 'regionEconomy', code: item.areaCode })),
                    );
                  });
                }
              }}
  ...
  }, [pageNum, data.hasIndicators, data.hasRanking, data.indexName, history, handleLimit]);
  ...
  return (
    <Modal
      ...
      getContainer={() => document.getElementById('area-areaf9-main-index-dom') || document.body}
      forceRender
    >
