...
import { Checkbox, Modal, Table } from '@/components/antd';
...
type PropsType = {
  ...
  // handleOpenModal?: (record: any, o: string, year?: number) => void;
};
...
const AreaInfoDialog: FC<PropsType> = ({
  ...
  const {
    state: { infoDetail, condition, openSource: listOpenSource, traceModalInfo },
  } = useCtx();
  ...
  const {
    ...
    handleTblCell,
  } = useUpdateTip({ defaultSource: listOpenSource, isLastMonth: true, missVCA: true });
  ...
  const exportCondition = useMemo(() => {
    return {
      ...
      endDate: '(*,*)',
      sort: 'endDate:desc',
      module_type: 'web_area_economy_area_info_new',
      exportFlag: true,
      isPost: true,
    };
  }, [condition?.indicName, infoDetail?.regionCode4]);
  ...
  const setting = useMemo(
    () => ({
      ...
      //fix bug35659,区域F9：辖区经济明细弹框点不开
      getContainer: () => document.querySelector('.dzh-container-right') as HTMLElement,
    }),
    [UpdateTipCref, handleCloseModal, hideUpdateTip, infoDetail.regionName],
  );
  ...
  return (
    <ModalStyle visible={show && !traceModalInfo?.visibel} {...setting}>
