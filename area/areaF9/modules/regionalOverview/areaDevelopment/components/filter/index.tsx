...
import { Options, quickAreaOptions, Screen, ScreenType } from '@/components/screen';
...
const Filter: FC<Props> = ({
  ...
  const { year: defaultYear } = useQuery();
  // const [setPageMenus] = useState<Options[]>([]);
  ...
  useEffect(() => {
    if (areaTree?.length) {
      let _codes: string[] = [];
      const regionList = findAreaJurisdictions({ list: areaTree, code });
      for (const item of regionList?.children || []) {
        _codes.push(item.value);
      }
      _codes.push(String(code));
  ...
  return (
    <Wrapper ref={domRef} key={filterKey}>
      <Screen options={yearConfig} onChange={yearChange} getPopContainer={popUpContainer} />
      {/* <Screen options={areaConfig} values={[initCodes]} onChange={areaChange} getPopContainer={popUpContainer} /> */}
      <TransferSelect
        title="指标"
        data={indicatorList}
