...
const useTreeChange = (rowDatas: any, isExpandAll: boolean, isEmptyLineOpen: boolean, gridRef: any) => {
  ...
  const traverseTree = useMemoizedFn((data: any, level = 1, title = '', isEmptyLineOpen: boolean) => {
    ...
    return flatMap(treeItem, (item) => {
      const { subIndicList, indicatorName, isEmpty, similarScoreList, ...rest } = item;
      /** 一级指标名称 */
      ...
      if (Array.isArray(similarScoreList) && similarScoreList.length > 0) {
        similarScoreList.forEach((item, i) => {
          curNode[`compareRegionName${i}`] = item.regionName;
          curNode[`compareRegionCode${i}`] = item.regionCode;
          curNode[`score${i}`] = item.score;
          curNode[`deviation${i}`] = item.deviation;
          curNode[`regionShortName${i}`] = item.regionShortName;
        });
      }
