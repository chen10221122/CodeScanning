...
import { getAreaScoreInitYear } from '@/apis/area/areaEconomy';
...
import useMenuConfig from './menuConfig2';
...
const Filters = forwardRef((_: Props, ref) => {
  const { administrationLevel, provinceCode, sameRegionLevel, onlyProvince, areaInfo } = _;
  ...
  const {
    ...
    loading,
    run: getYear,
  } = useRequest(() => getAreaScoreInitYear(params?.code), {
    manual: true,
    onFinally() {
      update((d) => {
        d.yearLoading = false;
  ...
  const exportCondition = useMemo(() => {
    return {
      module_type: 'area_f9_similar_score',
      isPost: true,
      exportFlag: true,
      year: params.year,
    ...
    <Filter ref={domRef}>
      <div className="left-part">
        ...
        <Popover
          ...
          getPopupContainer={() => document.getElementById('similarContainer2') || document.body}
        >
      ...
      <div className="right-part">
        ...
        <ExportDoc
          ...
          filename={`${areaInfo?.regionName}_相似评分_${dayjs(new Date()).format('YYYYMMDD')}`}
        />
      </div>
      <MetricModal
