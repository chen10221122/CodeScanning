...
import { Empty, Popover, Spin, Table } from '@/components/antd';
...
const SocialFinance: FC<any> = () => {
  ...
  useEffect(() => {
    if (code) {
      // 不是省就显示无数据
      setCurrentIsNotProvince(areaInfo?.level !== 1);
      // 参数变化用作导出
      updateParams((d) => ({
        ...d,
        regionCode: code,
  ...
  }, [areaInfo?.level, code, updateParams]);
  ...
  const exportOption = useMemo(
    () => ({
      ...params,
      regionalName: areaInfo?.regionName,
      startDate: '20130101',
              ...
              <>
                {
                  ...
                  originData?.length ? (
                    ...
                    <LgEmpty show={!loading} type={Empty.NO_DATA_LG} style={{ marginTop: '150px' }} />
                  )
                }
              </>
            ) : (
  ...
  return (
    <WrapperContainer
      ...
      headerRightContent={
        <>
          <div className="card-title" style={{ marginRight: '24px' }}>
            {filter}
          </div>
          <div className="select-right" style={{ marginTop: '-3px' }}>
            <Popover
              content={() => {
                return (
                  <div>
                    <div>
                      1. 地区社会融资规模增量同比：央行官网未直接披露同比值，财汇根据央行官网披露的增量值进行相应计算。
                    </div>
                    <div>
            ...
            >
              ...
              <span style={{ fontSize: 13, lineHeight: '20px', color: '#111', display: 'inline-block' }}>帮助说明</span>
            </Popover>
          </div>
