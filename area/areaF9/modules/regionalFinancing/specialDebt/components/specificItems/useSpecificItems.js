...
import { CONTAINS } from '../specialDebtProjects/useSpecialDebtProjects';

const PAGESIZE = 50;
export default function useSpecificItems({
  specialDebtLoading,
  setSearchType,
  setSelectedYear,
  selectedYear,
  ...
  const [condition, setCondition] = useState({
    ...
    skip: 0,
    pagesize: PAGESIZE,
    isQueryThisLevel: isSelfs === CONTAINS ? '2' : '1',
  });
  ...
  useEffect(() => {
    ...
    setCondition((originState) => {
      return { ...originState, skip: 0, isQueryThisLevel: isSelfs === CONTAINS ? '2' : '1' };
    });
    setFirstPageChange(false);
  }, [areaInfo?.level, isSelfs]);
