...
import { getLocalDebtNew } from '@/apis/area/areaEconomy';
import { useParams } from '@/pages/area/areaF9/hooks';
import { useAsync, useImmer } from '@/utils/hooks';
...
export default function useLocalDebtIssue() {
  ...
  const [condition, update] = useImmer({
    regionCode,
    dateScope: Frequency.MONTH,
  });
  ...
  const { data, execute, pending, error } = useAsync(getLocalDebtNew);
  ...
  return {
    pending,
    condition,
    columnsName: columnsName.current,
    chartData,
    tableData,
    error,
    execute,
    regionCode,
    frequency: condition.dateScope,
