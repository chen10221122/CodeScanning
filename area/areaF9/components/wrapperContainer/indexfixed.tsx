...
const WrapperContainer: FC<HeaderContentProps> = (props) => {
  ...
  useEffect(() => {
    const titleArr = [
      ...
      { path: LINK_AREA_SPREADS, title: '地区利差' },
  ...
  const Content = useMemo(() => {
    return (
      <>
        ...
        {backup && <BackTop target={() => containerRef?.current! || scrollWrapRef.current!} />}
      </>
    );
  }, [
    _title,
  ...
  return loadingHideContent ? (
    ...
    <>
      ...
      <Inner loading={loading}>{Content}</Inner>
    </>
  );
};
export default memo(WrapperContainer);

const Inner = styled.div<{ loading: boolean | undefined }>`
  height: 100%;
  overflow-y: ${({ loading }) => (loading ? 'hidden' : 'visible')};
  position: relative;
  z-index: 1;
`;
