...
const Table: FC<Props> = ({
    ...
    const getColumnItem = (item: indicatorsItem) => {
      ...
      item.render = (text: any, record: any) => {
        ...
        if (text) {
          ...
          /* 溯源 */
          if (isOpenSource && (record[title + '_guId'] || record[title + '_PosID'])) {
            if (record[title + '_PosID']) {
              result = (
                <span
                  style={{ color: '#025cdc', textDecoration: 'underline', cursor: 'pointer' }}
                  data-prevent // 必须
                  onClick={() => {
                    openDataSource && openDataSource({ posIDs: record[title + '_PosID'] });
                  }}
                >
                  {result}
                </span>
  ...
  }, [indicators, onSortChange, currentSort, setCurrentSort, isOpenSource, openDataSource]);
      ...
      let processedTableData = originData.map((item: any) => {
        const mergedObject = item?.hits.reduce((pre: any, cur: any) => {
          ...
          data = {
            ...
            [`${nameValueObj[indicName]}_PosID`]: cur.PosID || '',
          };
          return { ...pre, ...data };
        }, {});
  ...
  const columns = useMemo(
      ...
      },
      {
        ...
        render: (text: any, row: any) => (
          <Link
            ...
            className="link"
            to={urlJoin(
              dynamicLink(LINK_AREA_F9, { key: 'regionEconomy', code: row?.['CR0231_013'] }),
              urlQueriesSerialize({
                code: row?.['CR0231_013'],
              }),
            )}
    ...
    [curentPage, indicatorColumns, paginationSize, searchKey],
  );
  ...
  return (
    <TableContainer tableHeadTopHeight={tableHeadTopHeight}>
      ...
      <DetailInfoDialog
        ...
        close={() => setDetailModalShow(false)}
