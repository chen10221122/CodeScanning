...
interface PropsType {
  ...
  // /** 外部页面的溯源是否开启，11418需求新增：弹窗内部的溯源默认状态跟随外部页面 */
  // defaultSource?: boolean
  openDataSource?: OpenDataSourceDrawer;
}

const DetailInfoDialog: FC<PropsType> = ({
  detailInfo,
  openDataSource,
  show,
  close,
  container /** defaultSource = false */,
}) => {
      ...
      <div className="modal_title">
        ...
        {handledTableData.length ? (
          <div className="title-filter">
            ...
            {traceCref}
            <ExportDoc
              filename={`${detailInfo?.devzName}历年明细_${dayjs(new Date()).format('YYYYMMDD')}`}
              condition={{
                devzCode: detailInfo?.DEVZCode,
                specialType: 1,
                module_type: 'devZone_devZoneContrastDetail',
                exportFlag: 'true',
                sheetNames: { '0': '开发区历年明细' },
                exportType: '1',
              }}
            />
          </div>
        ...
        result.columns.push({
          ...
          render: (text, record) => {
            ...
            if (text) {
              result = text;
              /* 溯源 */
              if (traceSource && (record[year + '_guId'] || record[year + '_PosID'])) {
                if (record[year + '_PosID']) {
                  result = (
                    <span
                      style={{ color: '#025cdc', textDecoration: 'underline', cursor: 'pointer' }}
                      data-prevent // 必须
                      onClick={() => {
                        openDataSource && openDataSource({ posIDs: record[year + '_PosID'] });
                      }}
                    >
                      {result}
                    </span>
  ...
  }, [handledTableData, years, check, traceSource, openDataSource]);
