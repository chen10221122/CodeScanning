...
import useColumns from '@/pages/area/areaCompany/module/entityRating/useColumns';
import { useSelector } from '@/pages/area/areaF9/context';
import { useImmer } from '@/utils/hooks';
import { shortId } from '@/utils/share';

import SearchLine from './components/searchLine';
import { handleSelectData } from './components/searchLine/screenConf';
...
export default () => {
  ...
  const columns = useColumns({ page: 'entityRating', condition });
  ...
  const tabParams = useMemo(() => {
    ...
    switch (activeKey) {
      case 'all':
        tabResParams = {
          unionQueryParams: 'raitingOutlook:3,ratingChangeDirection:2,ratingZWChangeDirection:2',
        };
        break;
      case 'subjectDown':
        tabResParams = {
  ...
  // 翻页
  const handlePageChg = useMemoizedFn((x: any) => {
    update((d) => {
      d.skip = (x - 1) * PAGE_SIZE;
    });
  });
      ...
      switch (type) {
        ...
        case 'screen': {
          ...
          if (isArray(yearRange?.value)) {
            startRatingDate = dayjs(yearRange?.value?.[0] || '').format('YYYYMMDD');
            endRatingDate = dayjs(yearRange?.value?.[1] || '').format('YYYYMMDD');
          ...
          if (startDate && endDate) {
            startRatingDate = dayjs(startDate).format('YYYYMMDD');
            endRatingDate = dayjs(endDate).format('YYYYMMDD');
          }
          /** 债券违约上古方法 */
    ...
    >
      ...
      {listInfo?.data?.length ? (
        ...
        <Spin
          ...
          useTag
          keepCenter
          wrapperClassName="empty-mount-table-loading-animation"
        >
          <Empty
