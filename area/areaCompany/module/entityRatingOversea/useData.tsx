...
import { getOverseaEntityRating } from '@/pages/area/areaCompany/api/regionFinancingApi';
...
export default ({ condition, tabParams }: { condition: TObj; tabParams: any }) => {
  ...
  const { loading, run, error } = useRequest(getOverseaEntityRating, {
    manual: true,
    onSuccess(res: TObj) {
      setListInfo({ total: res?.data?.total ?? '', data: res?.data?.data });
    },
    onError(err: any) {
