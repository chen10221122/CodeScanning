...
interface DetailModalProps {
  visible: boolean;
  detailModalConfig: { modalType: DetailModalTypeEnum; title: string; defaultCondition?: any };
  closeModal: () => void;
  /** 挂载dom */
  containerRef: any;
}
const DetailModal = ({ visible, detailModalConfig, closeModal, containerRef }: DetailModalProps) => {
  ...
  // 根据不同弹窗类型获取不同数据
  const { isFirstLoadRef, page, setPage, handleChangePage, dataSource, count, loading, run } = useModalData({
    visible,
    detailModalConfig,
  });
  // 不同弹窗类型不同筛选项
  ...
  const showRemark = useMemo(() => {
    return [DetailModalTypeEnum.NewThirdAdd, DetailModalTypeEnum.StockARefinance].includes(detailModalConfig.modalType);
    ...
    return (
      <div className={S.modalTitle}>
        <div className={S.title}>
          <div>{detailModalConfig.title || DetailModalInfoMap.get(detailModalConfig.modalType)?.title}</div>
        </div>
        {!isLoading ? (
...
const TableContainer = styled.div<{ hasBottom: boolean; noScreen: boolean }>`
  height: ${({ hasBottom, noScreen }) =>
    hasBottom ? `calc(100vh - ${noScreen ? 202 : 246}px)` : `calc(100vh - ${noScreen ? 200 - 30 : 200}px)`};
  padding: 0 32px;
  overflow-y: auto;
